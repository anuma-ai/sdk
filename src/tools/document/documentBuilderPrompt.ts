...
- Root is exactly one <Document>. Its only children are <Page> elements. Add more <Page> elements for longer documents — react-pdf also paginates overflow automatically.
...
- <Text> may nest <Text> or <Link> for inline styling (bold runs, links). <Image> is self-closing and its src MUST be an inline "data:" URI — remote image URLs are rejected for privacy. Vector art uses <Svg> with <Rect>/<Circle>/<Path>/etc.
