...
import { FONT_PRESETS, isKnownFont, renderFontLibrary } from "./fonts";

export type { FontCategory, FontPreset, FontSpec } from "./fonts";
export { buildFontsUrl, FONT_LIBRARY, FONT_PRESETS, getFontByName, isKnownFont } from "./fonts";
...
import { getPaletteByName, renderPaletteColors } from "./palettes";
...
// FontPreset + FONT_PRESETS now live in ./fonts (a dependency-free module) so
// the slide persona builder (./slidePrompt) and the lib tool-set layer can
// import them without pulling this module's @babel/parser-bearing JSX deps.
// Imported above for internal use and re-exported above for back-compat.
...
// buildSlideSystemPrompt + BuildSlideSystemPromptOptions + SLIDE_BUILDER_PROMPT
// live in ./slidePrompt — a module with no edge to ./jsx (@babel/parser) — so
// the lib tool-set layer can attach the slide persona to the `slides` set's
// systemPrompt without pulling this module's JSX deps into that bundle.
// Re-exported here for the public API (src/tools/index.ts) and host call sites.
export type { BuildSlideSystemPromptOptions } from "./slidePrompt";
export { buildSlideSystemPrompt, SLIDE_BUILDER_PROMPT } from "./slidePrompt";
