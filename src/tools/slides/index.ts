...
// ---------------------------------------------------------------------------
// Recommended maxToolRounds for slide deck generation
// ---------------------------------------------------------------------------

/**
 * Recommended `maxToolRounds` value when using slide deck tools.
 *
 * A full deck flow needs 1 plan_deck + up to 19 add_slide calls = 20 rounds,
 * plus a small margin for the final text response. Pass this to `runToolLoop`
 * or `useChat` when slide tools are active:
 *
 * ```ts
 * await sendMessage({ tools: slideTools, maxToolRounds: SLIDE_MAX_TOOL_ROUNDS });
 * ```
 */
export const SLIDE_MAX_TOOL_ROUNDS = 22;
  ...
  arguments: {
    ...
    properties: {
      ...
      slideCount: {
        type: "integer",
        minimum: 3,
        // Capped at 19: a full deck is 1 plan_deck + N add_slide calls.
        // Callers must pass `maxToolRounds: SLIDE_MAX_TOOL_ROUNDS` (22) to
        // support the full per-slide flow; the default maxToolRounds of 3 is
        // intentionally low for non-slide callers.
        maximum: 19,
        description:
...
1. TELL THE USER — Before calling any tools, send a short text message: "Generating a deck on <topic> — this can take 2–3 minutes." Replace <topic> with the subject the user asked about.
