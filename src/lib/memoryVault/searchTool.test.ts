...
const mockStore: MemoryStoreReader = {
  getAll: vi.fn(),
  getById: vi.fn(),
};
...
describe("searchVaultMemories", () => {
  ...
  it("returns structured VaultSearchResult[] sorted by similarity", async () => {
    ...
    vi.mocked(mockStore.getAll).mockResolvedValue(memories);
    ...
    const results = await searchVaultMemories("cats", mockStore, mockEmbeddingOptions, cache, {
      minSimilarity: 0,
    });
  ...
  it("returns [] for empty vault", async () => {
    ...
    const cache = createVaultEmbeddingCache();
    const results = await searchVaultMemories("test", mockStore, mockEmbeddingOptions, cache);
  ...
  it("returns [] for empty query", async () => {
    const cache = createVaultEmbeddingCache();
    const results = await searchVaultMemories("", mockStore, mockEmbeddingOptions, cache);
  ...
  it("respects scopes — only returns memories matching given scopes", async () => {
    ...
    await searchVaultMemories("test", mockStore, mockEmbeddingOptions, cache, {
      scopes: ["private"],
    });
...
describe("createMemoryVaultSearchTool", () => {
  ...
  it("ranks results by cosine similarity (highest first)", async () => {
    ...
    const tool = createMemoryVaultSearchTool(mockStore, mockEmbeddingOptions, cache, {
      minSimilarity: 0,
    });
  ...
  it("returns 'no relevant memories' when all results are below threshold", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([makeMemory("m1", "unrelated")]);
    vi.mocked(generateEmbedding).mockResolvedValue([1, 0, 0]);
  ...
  it("passes scopes to store.getAll when configured", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([]);
  ...
  it("calls store.getAll with no scopes when not configured", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([]);
    ...
    expect(mockStore.getAll).toHaveBeenCalledWith(undefined);
  });

  it("returns error message when embedding generation fails", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([makeMemory("m1", "content")]);
...
describe("preEmbedVaultMemories", () => {
  ...
  it("passes scopes through to store.getAll", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([]);

    const cache = createVaultEmbeddingCache();
    await preEmbedVaultMemories(mockStore, mockEmbeddingOptions, cache, {
  ...
  it("calls store.getAll with undefined when no options", async () => {
    vi.mocked(mockStore.getAll).mockResolvedValue([]);
