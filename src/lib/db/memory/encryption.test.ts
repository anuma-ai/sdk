...
describe("Memory Encryption Utilities", () => {
  ...
  describe("Deterministic Encryption", () => {
    ...
    it("should use deterministic encryption for namespace, key, and value in encryptMemoryFields", async () => {
      await requestEncryptionKey(testAddress, mockSignMessage);
      ...
      // Queryable fields (namespace, key, value) should be deterministically encrypted
      ...
      // rawEvidence uses random IV, so may differ (but that's okay, it's not queried)
      expect(isEncrypted(encrypted1.rawEvidence)).toBe(true);
      expect(isEncrypted(encrypted2.rawEvidence)).toBe(true);
    });
  });
