...
import type { Budget, MemoryKind, RecallContext, RecallOptions } from "./types.js";
...
function formatRecallResult(
  ...
  const lines = memories.map((m, i) => {
    const score = (m.scoreBreakdown?.cosine ?? m.score).toFixed(2);
    if (m.kind === "fact") {
...
export function createRecallTool(
  ...
  return {
    ...
    executor: async (args: Record<string, unknown>): Promise<string> => {
      ...
      const requestLimit = (args.limit as number) ?? defaultLimit;
