...
- The agent server never persists user content (messages, files, subscription data) beyond the lifetime of a turn.
- Session-scoped in-memory state is dropped when the turn ends (or the session idles out).
- The launching client's local store remains canonical for everything user-owned.
- Agent server persistent state is limited to: OAuth refresh tokens, scheduled job definitions, minimal session metadata (session_id, agent_id, user_id, created_at).

## Rollout order

Ordered by risk and complexity:

1. **Sentinel first** — MCP-only, no custom tools, simplest. Proves the pipeline end-to-end (OAuth, SSE, portal bearer auth).
2. **App Creator second** — exercises custom-tool migration, the snapshot-and-mutate pattern, and the `render_instruction` SSE event.
