...
| `createPlatformCursorStore` | client-mounted | #768 C1, landed in zeta-chain/ai-memoryless-client#5175. Web: `apps/web/hooks/useAutoExtraction.ts:96` over `webPlatformStorage`. Mobile: `apps/mobile/hooks/useAutoExtraction.ts:103` over an AsyncStorage-backed cache (`utils/extractionCursorStore.ts`). Both feed `createAutoExtractor`, so the extraction watermark now survives a reload. |
